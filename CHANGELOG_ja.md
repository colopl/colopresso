...
- **変更:** `--type limited` では、視覚重要度マップ・高彩度アンカー・適応ディザリング・グラデーションブースト・
  彩度重み付け・平滑化を未指定時は無効にし、有効化オプション (`--saliency-map`, `--chroma-anchor`,
  `--adaptive-dither`, `--gradient-boost`, `--chroma-weight`, `--smooth`) が指定された場合はエラーにします。Limited RGBA4444 はこれらを一切使用しないためです。
